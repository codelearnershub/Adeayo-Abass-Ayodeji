    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            for (int i = 0; i < numbers2.Length; i++)
            {
                if (i==0)
                {
                    sum1 = numbers2[x] + numbers1[2];
                }
                else
                {
                    sum2 = numbers1[1] + numbers2[1];
                }

                x++;
            }
            Polynomials(numbers1, numbers2, sum1, sum2, num1, num2, num3);
        ...
        static void Polynomials(int[] numbers1, int [] numbers2, int sum1, int sum2, int num1, int num2, int num3)
        {
            ...
            if (sum2 <0 && sum1<0)
            {
                Console.Write($"The sum of polynomials ({num3}x^2 + {num2}x -{num1}) + ({num2}x -{num1} = {num3}x^2 {sum2}x {sum1}");
            }
